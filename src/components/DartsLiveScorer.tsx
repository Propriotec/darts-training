...
const LADDER: Record<number, number> = { 1: 40, 2: 50, 3: 60, 4: 70 };
...
function loadData() {
  try {
    ...
    return r ? JSON.parse(r) : emptyData();
  } catch {
    return emptyData();
  }
}
function saveData(data: ReturnType<typeof emptyData>) {
  try {
...
function Rules({ title, children }: { title: string; children: React.ReactNode }) {
...
/* ═══════════════════════════════════
   TONS — dart by dart
   ═══════════════════════════════════ */
function Tons({ onSave }: { onSave: (d: any) => void }) {
  const [cur, setCur] = useState<string[]>([]);
  const [log, setLog] = useState<any[]>([]);
  const [darts, setDarts] = useState(0);
  const vals: Record<string, number> = { S: 20, D: 40, T: 60, X: 0 };
  ...
  const valid = log.filter((v: any) => v.ok);
  const c60 = valid.filter((v: any) => v.score >= 60 && v.score < 100).length;
  const c100 = valid.filter((v: any) => v.score >= 100 && v.score < 140).length;
  const c140 = valid.filter((v: any) => v.score >= 140).length;
  const pts = c60 + c100 * 2 + c140 * 3;
  const done = pts >= 21;
  const label: Record<string, string> = { S: "20", D: "D20", T: "T20", X: "Miss" };
  const color: Record<string, string> = { S: "#059669", D: "#2563eb", T: "#7c3aed", X: "#dc2626" };
      ...
      {log.length > 0 && (
        <div style={{ ...S.card, padding: 0, overflow: "hidden" }}>
          ...
          <div style={{ maxHeight: 180, overflowY: "auto" }}>
            {[...log].reverse().map((v: any, idx: number) => {
              ...
              return (
                <div key={idx} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "6px 12px", borderBottom: "1px solid #f3f4f6", opacity: v.ok ? 1 : 0.4 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                    ...
                    {v.d.map((d: string, di: number) => (<span key={di} style={{ padding: "2px 5px", borderRadius: 6, fontSize: 10, fontWeight: 700, background: d === "X" ? "#fef2f2" : d === "T" ? "#f3e8ff" : d === "D" ? "#eff6ff" : "#ecfdf5", color: color[d] }}>{label[d]}</span>))}
                  </div>
...
/* ═══════════════════════════════════
   LADDER UP
   ═══════════════════════════════════ */
function LadderUp({ week, onSave }: { week: number; onSave: (d: any) => void }) {
  ...
  useEffect(() => { setCur(start); setHi(start); setAtt(0); setHits(0); setTime(1200); setOn(false); if (ref.current) clearInterval(ref.current); }, [week, start]);
...
/* ═══════════════════════════════════
   ...
   ═══════════════════════════════════ */
function JDCGame({ onSave }: { onSave: (d: any) => void }) {
        ...
        {nums.map((n, i) => {
          ...
          return (
            <div key={i} style={{
              ...
              boxShadow: active ? "0 0 0 2px rgba(124,58,237,0.2)" : (done ? "none" : "none")
            }}>{hit ? "✓" : lbl}</div>
          );
        })}
      </div>
...
function PhaseDone({ label, score, target, results, nums, shanghais, isDoubles }: { label: string; score: number; target: number; results: any; nums: (number | string)[]; shanghais?: number; isDoubles?: boolean }) {
  return (
    <div style={{ ...S.card, background: score >= target ? "#f0fdf4" : "#f9fafb" }}>
      ...
      {isDoubles ? (
        ...
        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
          {nums.map(n => {
            ...
            if (!r) return null;
            return (
              <div key={String(n)} style={{ padding: "4px 10px", borderRadius: 10, background: r.shanghai ? "#fef9c3" : "#f3e8ff", fontSize: 11, fontWeight: 700, color: r.shanghai ? "#92400e" : "#5b21b6" }}>
      ...
      {shanghais !== undefined && shanghais > 0 && <div style={{ marginTop: 6, fontSize: 12, fontWeight: 700, color: "#a16207" }}>{shanghais} Shanghai{shanghais > 1 ? "s" : ""}!</div>}
...
function History({ data }: { data: any }) {
  ...
  const weekAvg = (w: any[], f: string) => { const v = w.filter((d: any) => d).map((d: any) => d[f]); return v.length ? v.reduce((a: number, b: number) => a + b, 0) / v.length : null; };
  const fmt = (v: any, dec = 0) => v !== null && v !== undefined ? (dec > 0 ? Number(v).toFixed(dec) : Math.round(v)) : "-";
  const trend = (vals: any[]) => { const f = vals.filter((v: any) => v !== null); if (f.length < 2) return null; return f[f.length - 1] > f[0] ? "up" : f[f.length - 1] < f[0] ? "down" : "same"; };

  const MiniGrid = ({ title, color, weeks, renderCell, summaryRows }: { title: string; color: string; weeks: any[][]; renderCell: (d: any) => React.ReactNode; summaryRows: any[] }) => (
    <div style={{ ...S.card, padding: 0, overflow: "hidden" }}>
  ...
  const pill = (v: any, good: boolean): React.CSSProperties => ({ display: "inline-block", padding: "2px 5px", borderRadius: 6, fontSize: 10, fontWeight: 700, background: v === null ? "transparent" : good ? "#ecfdf5" : "#f3f4f6", color: v === null ? "#d1d5db" : good ? "#059669" : "#374151" });
...
export default function DartsLiveScorer() {
  ...
  const nextSlot = (game: string) => { const w = (data as any)[game][wk - 1]; const s = w.findIndex((d: any) => d === null); return s >= 0 ? s : -1; };

  const addSession = (game: string, sessionData: any) => {
    const nd = JSON.parse(JSON.stringify(data));
    const wi = wk - 1;
    let slot = nd[game][wi].findIndex((d: any) => d === null);
    if (slot === -1) {
  ...
  const clearAll = () => { const nd = emptyData(); setData(nd); saveData(nd); };
    ...
    <div style={{ maxWidth: 480, margin: "0 auto", background: "#f3f4f6", minHeight: "100vh", paddingBottom: 40 }}>
      <div style={{ background: "linear-gradient(135deg, #111827 0%, #1e293b 100%)", padding: "16px 0 0", textAlign: "center", position: "relative" }}>
        ...
        <div style={{ display: "flex", gap: 6, justifyContent: "center", padding: "10px 0" }}>
          {[1, 2, 3, 4].map(w => {
            const filled = data.tons[w - 1].filter((d: any) => d !== null).length;
            return (
      ...
      <div style={{ padding: 12 }}>
        {tab === "tons" && <Tons onSave={(d: any) => addSession("tons", d)} />}
        {tab === "ladder" && <LadderUp week={wk} onSave={(d: any) => addSession("ladder", d)} />}
        {tab === "jdc" && <JDCGame onSave={(d: any) => addSession("jdc", d)} />}
